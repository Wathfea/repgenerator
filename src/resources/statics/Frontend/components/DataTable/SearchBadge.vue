<template>
  <span class="inline-flex rounded-lg items-center px-2 pr-1.5 py-1 text-sm font-medium bg-white border-2 border-repgenerator-800 text-repgenerator-700" :style="{ lineHeight: '21px'}">
    <SearchCircleIcon @click="onOpenSearch" class="h-5 w-5 text-repgenerator-800 mr-1 hover:text-repgenerator-700 cursor-pointer" aria-hidden="true"/>
    <b class="pr-1">{{ name + ":" }}</b>
    <svg v-if="Array.isArray(data.data.values) && !data.data.values.length" class="animate-spin m-auto h-4 w-4 text-repgenerator-800" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <Button @click="onRemove" class="pl-1 pr-0 py-0 bg-transparent hover:bg-transparent">
       <XIcon class="h-4 w-4 text-repgenerator-800 hover:text-repgenerator-700" aria-hidden="true"/>
    </Button>
  </span>
</template>
<script setup>
